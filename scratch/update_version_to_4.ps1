# Increment version from 3.28.3 to 3.28.4 in index.html and portfolio.html
...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        $newHtml = $html.Replace("3.28.3", "3.28.4")
        
        [System.IO.File]::WriteAllText($file, $newHtml, [System.Text.Encoding]::UTF8)
        Write-Host "Updated version to 3.28.4 in $file"
    } else {
